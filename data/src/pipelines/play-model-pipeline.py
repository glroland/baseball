...
run_notebook_out_of_proc = components.load_component_from_file('run-notebook-out-of-proc-component.yaml')

@dsl.pipeline(name="Play Prediction Model Lifecycle Pipeline")
def train_model_pipeline(git_url: str, db_conn_str: str):
    # Train Model
    run_task = run_notebook_out_of_proc(git_url=git_url,
                                        run_from_dir="data/src/train",
                                        notebook_name="train_predict_play_model.ipynb",
                                        db_conn_str=db_conn_str,
                                        parameters = {
                                        })
    run_task.set_display_name("train-play-model")
